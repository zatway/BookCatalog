...
{
    internal class AddSearchBooksFunction : IMigrator
    {
        private readonly string _searchBooksSql = @"
...
BEGIN
    ...
    ORDER BY
        CASE
            ...
            WHEN selected_filter = 'По году выпуска' THEN to_char(b.year_of_manufacture, 'YYYY')
            ELSE b.id::text  -- Используем идентификатор по умолчанию
        END;
END;
$$ LANGUAGE plpgsql;";
        ...
        public void Migrate(string? targetMigration = null)
        {
            ...
            {
                dbContext.Database.ExecuteSqlRaw(_searchBooksSql);
            }
        }
        ...
        public async Task MigrateAsync(string? targetMigration = null, CancellationToken cancellationToken = default)
        {
            using (var dbContext = new MyDbContext())
            {
                await dbContext.Database.ExecuteSqlRawAsync(_searchBooksSql, cancellationToken);
            }
        }
